...
it("selection.attr(name) returns the value of the attribute with the specified name on the first selected element", () => {
  ...
  assert.strictEqual(d3.select(document).select("h1").attr("class"), "c1 c2");
  assert.strictEqual(d3.selectAll([null, document]).select("h1").attr("class"), "c1 c2");
});

it("selection.attr(name) coerces the specified name to a string", () => {
  const document = jsdom("<h1 class='c1 c2'>hello</h1><h1 class='c3'></h1>");
  assert.strictEqual(d3.select(document).select("h1").attr({toString: function() { return "class"; }}), "c1 c2");
});
...
it("selection.attr(name, value) sets the value of the attribute with the specified name on the selected elements", () => {
  ...
  assert.strictEqual(selection.attr("foo", "bar"), selection);
...
it("selection.attr(name, null) removes the attribute with the specified name on the selected elements", () => {
  const document = jsdom("<h1 id='one' foo='bar' class='c1 c2'>hello</h1><h1 id='two' foo='bar' class='c3'></h1>"),
      ...
      two = document.querySelector("#two"),
      selection = d3.selectAll([one, two]);
  assert.strictEqual(selection.attr("foo", null), selection);
