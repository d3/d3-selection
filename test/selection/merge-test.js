...
it("selection.merge(selection) returns a new selection, merging the two selections", () => {
  ...
  assert.deepEqual(selection1.merge(selection2), {_groups: [[one, two]], _parents: [body]});
  assert.deepEqual(selection1, {_groups: [[, two]], _parents: [body]});
  assert.deepEqual(selection2, {_groups: [[one, ]], _parents: [body]});
});
...
it("selection.merge(selection) returns a selection with the same size and parents as this selection", () => {
  ...
  assert.deepEqual(d3.select(body0).selectAll("h1").merge(d3.select(body1).selectAll("h1")), {_groups: [[one0, two0]], _parents: [body0]});
  assert.deepEqual(d3.select(body1).selectAll("h1").merge(d3.select(body0).selectAll("h1")), {_groups: [[one1, two1, three1]], _parents: [body1]});
});
...
it("selection.merge(selection) reuses groups from this selection if the other selection has fewer groups", () => {
  ...
  assert.deepEqual(selection01, selection0);
  assert.deepEqual(selection10, selection1);
