...
Selection.prototype = {
  ...
  select: function(selector) {
    ...
    return new Selection(select(this._root, depth), depth);
  },
  ...
  selectAll: function(selector) {
    ...
    function selectAll(nodes, depth) {
      var i = -1,
          n = nodes.length,
          node,
          subnode,
      ...
      if (stack0) while (++i < n) {
        node = nodes[i];
        stack[stack0] = node._parent.__data__, stack[stack1] = i;
        subnodes[i] = subnode = selectAll(node, depth);
    ...
    return new Selection(selectAll(this._root, depth), depth + 1);
    ...
    //   if (nodes._leaf) while (++i < n) {
    //     if (node = nodes[i]) {
    //       stack[0] = node.__data__;
    //       stack[1] = i;
    //       subnodes[i] = subnode = selector.apply(node, stack);
    //       subnode._parent = node;
    //       subnode._leaf = true;
    ...
    //   else while (++i < n) {
    //     node = nodes[i];
    //     stack[0] = node._parent.__data__;
    //     stack[1] = i;
    //     subnodes[i] = subnode = selectAll(node);
    //     subnode._parent = node._parent;
    //     subnode._leaf = false;
    //   }
    ...
    // return new Selection(selectAll(this._root), this._depth + 1);
  },
  ...
  data: function(value, key) {
    ...
    function data(nodes) {
      if (nodes._leaf) {
        return void bind(nodes, typeof value === "function"
            ? value.apply(nodes._parent, stack)
            : value);
      }
    ...
    function bindByIndex(group, groupData) {
    //     for (i = -1; ++i < n0;) {
    //       node = group[i];
...
// function EnterSelection(groups, update) {
