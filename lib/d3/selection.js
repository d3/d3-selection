...
Selection.prototype = {
  ...
  data: function(value, key) {
    var depth = this._depth - 1,
        ...
        enter = this.enter(), // initializes _enter and _update references
        exit = this.exit(); // initializes _exit references
    ...
    function visit(nodes, depth) {
      ...
      if (depth) {
        ...
        bind(nodes, value.apply(nodes, stack));
      }
    ...
    function bindByIndex(update, data) {
      ...
      // runs when dataLength is greater than nodeLength, and thus update[i] is
      // guaranteed not to exist.
      for (; i < dataLength; ++i) {
        enter[i] = new EnterNode(data[i]);
      }

      // Note: we don’t need to delete update[i] here because immediately
      // following this loop, we set the update.length to the data length, which
      // will truncate any update nodes as needed.
      for (; i < nodeLength; ++i) {
        if (node = update[i]) {
          exit[i] = update[i];
        }
