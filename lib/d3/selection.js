        ...
        while (++i < n) {
          if (node = nodes[i]) {
            ...
            if (subnode = selector.apply(node, stack)) {
              ...
              if (nodes._update) nodes._update[i] = subnode; // TODO hrm
              subnodes[i] = subnode;
            }
          }
        }
...
// // XXX This doesn’t override selection.insert.
// EnterSelection.prototype.append = function(name, before) {
//   if (arguments.length < 2) before = selectorUpdateOf(this);
//   return Selection.prototype.append.call(this, name, before);
// };
...
EnterNode.prototype = {
  appendChild: function(child) {
    return this._parent.appendChild(child);
  },
  insertBefore: function(child, after) {
    return this._parent.insertBefore(child, after);
  }
};
