...
// When depth = 2, root = [[Node, …], [Node, …], …].
// When depth = 3, root = [[[Node, …], …], …]. etc.
// Note that [Node, …] and NodeList are used interchangeably.
...
Selection.prototype = {
  ...
  sort: function(comparator) {
    ...
    function visit(nodes, depth) {
      if (--depth) {
        var i = -1,
            n = nodes.length;
        while (++i < n) {
          visit(nodes[i] = arrayOf(nodes[i]), depth);
        }
      }
    ...
    visit(this._root = arrayOf(this._root), this._depth);
    return this.order();
  },
